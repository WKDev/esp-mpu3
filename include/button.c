...
static void button_fire_event(button_t *button)
{
    ...
    case 2:
        ...
        break;
        // case 3:
        //     event = button_event_tripple_press;
        //     break;
    }
...
static void button_toggle_callback(bool high, void *context)
{
    ...
    else
    {
        ...
        if (button->long_press_timer && xTimerIsTimerActive(button->long_press_timer))
        {
            xTimerStop(button->long_press_timer, 1);
        }

        if (button->press_count >= button->config.max_repeat_presses || !button->config.repeat_press_timeout)
        {
